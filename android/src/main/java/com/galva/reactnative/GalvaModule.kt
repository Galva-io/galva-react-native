 ...
 *
 * PHASE-3 STUB (plan §3.9 / §7): the Android core (Galva-io/galva-android)
 * exists but is unreleased (`1.0.0-SNAPSHOT`, coordinate unsettled), so every
 * method here is a stub — void calls no-op, getters resolve neutral defaults,
 * `show` rejects — each logging once so the gap is visible, never a silent
 * success pretending to be backed. When `io.galva…:galva-sdk:1.0.0` ships,
 * these delegate to the Maven AAR (no source vendoring on Android).
 */
class GalvaModule(reactContext: ReactApplicationContext) :
  ...
  private fun stub(method: String) {
    if (warned.add(method)) {
      Log.w(NAME, "$method(): Android core not released yet — call is a no-op (plan §3.9).")
    }
  }
  ...
  fun show(messageId: String, promise: Promise) {
    promise.reject(
      "NOT_IMPLEMENTED",
      "Galva.show(): the Android core is not released yet (plan §3.9) — in-app messages are iOS-only for now."
    )
  }
